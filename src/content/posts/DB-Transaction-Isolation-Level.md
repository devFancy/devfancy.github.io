...
* T4: 트랜잭션 A가 처리를 마치고 `5,000원`이 차감된 잔액을 <strong>커밋(Commit)</strong>한다. DB의 실제 잔액은 `5,000원`이 되고 Lock은 해제된다. 대기하던 트랜잭션 B가 드디어 Lock을 획득하고 깨어난다.
...
- `READ COMMITTED`: 다른 트랜잭션의 변경 사항을 빠르게 반영해야 하는 금융 정보 조회 등 <strong>데이터 정합성(최신 데이터 반영)</strong>이 중요할 때 유리하다. (ex: 계좌 잔액)
