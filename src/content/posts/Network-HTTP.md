...
* IP 기반이 아닌 <strong>연결 별 고유 UUID(Connection ID)</strong>를 이용해 각 연결들을 식별한다. `TCP` 기반에서는 Wi-Fi에서 셀룰러 환경으로 이동하면 **IP 주소가 변경되어 연결을 재수립**해야 하지만, `QUIC`에서는 ID 기반이기 때문에 **연결이 그대로 유지**된다.
