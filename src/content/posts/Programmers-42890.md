...
1. 깊이가 목표 깊이에 도달했을 때:
    ...
    - <strong>`depth`</strong>가 <strong>`end`</strong>와 같아지면, 즉, 조합이 완성된 경우입니다.
...
2. 조합 확인 및 유일성 검사:
    ...
    - 선택된 열의 인덱스를 <strong>`list`</strong>에 저장하고, <strong>`key`</strong>에는 해당 인덱스를 문자열로 연결합니다.
3. 중복 체크를 위한 Map 생성 및 유일성 검사:
    ...
    - 조합을 이용하여 만들어진 문자열 <strong>`s`</strong>를 이용해 <strong>`map`</strong>에 저장하여 유일성을 체크합니다.
...
4. 최소성 검사 및 후보키 추가:
    ...
    - <strong>`count`</strong>를 통해 일치하는 문자열의 개수를 셉니다.
    - 만약 <strong>`count`</strong>가 현재 후보키의 길이와 같다면, 현재 후보키는 이미 저장된 후보키의 일부이므로 함수를 종료합니다.
