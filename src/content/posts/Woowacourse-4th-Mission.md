...
* <strong>이번 주차 과제를 하면서 가장 어려웠던 부분이 `에러 처리`하는 부분과 `클래스 분리`</strong>였습니다.
...
* 이번 미션를 완수하면서 느낀 점은 함수의 역할이 이전보다 더 명확해서 '제 3자가 저의 코드를 봐도 이해할 수 있겠구나' 라는 생각이 들었습니다. <strong>'다른 사람이 내 코드를 봐도 충분히 이해할 수 있는 코드를 만드는 것'</strong>이 중요하다는 걸 다시 알게 되었습니다.
