...
![](/assets/img/server/technology/daanggn/Daanggn-Tech-MeetUp-2024-Ops-Dev-Team-1.png)
...
![](/assets/img/server/technology/daanggn/Daanggn-Tech-MeetUp-2024-Ops-Dev-Team-2.png)
...
* API를 개선하고 확장성을 확보하기 위해 기존의 거대한 API를 **각 서비스에 맞는 API로 분리**한다.
    ...
    * 하지만 각 서비스 정보 조회 API를 보면, 여전히 너무 많은 코드가 API 메서드 1개에 작동되고 있다.
    ...
    ![](/assets/img/server/technology/daanggn/Daanggn-Tech-MeetUp-2024-Ops-Dev-Team-3.png)
...
* 각 서비스별로 사용자 정보 조회 API 내부에서 **역할** 에 따라 분리한다.
    ...
    * 역할이 분리하고 난 뒤에 다른 서비스에 있는 `사용자 정보 조회` 를 봤더니, 결국에는 모든 API가 비슷한 형태로 된 것을 확인할 수 있다.
    ...
    ![](/assets/img/server/technology/daanggn/Daanggn-Tech-MeetUp-2024-Ops-Dev-Team-4.png)
...
* 분리되어 있던 각 서비스별 사용자 정보 조회 API를 하나의 API로 통합하기로 한다. (API 형태가 비슷하기 때문)
    ...
    * 하지만 이는 서비스가 추가될 때마다 이 분기문 길이가 길어지면서 다시 코드가 복잡해지는 문제가 발생한다.
    ...
    ![](/assets/img/server/technology/daanggn/Daanggn-Tech-MeetUp-2024-Ops-Dev-Team-5.png)
...
* 위의 문제를 해결하기 위해 `메타 프로그래밍` 기법을 도입한다.
    ...
    * 여러 언어(JavaScript, Java, C++ 등)에서도 이 `메타 프로그래밍` 을 쓸 수 있다.
    ...
    ![](/assets/img/server/technology/daanggn/Daanggn-Tech-MeetUp-2024-Ops-Dev-Team-6.png)
    ...
    * 새로운 서비스를 추가하더라도 한 번 API 호출 할 때 하나의 서비스 정보만 불러오기 때문에, 응답 시간에는 영향을 미치지 않는다. 이는 확장성의 의미인 **최소한의 사이드 이펙트** 로 새로운 기능을 추가할 수 있는 상태가 되었다.
    ...
    ![](/assets/img/server/technology/daanggn/Daanggn-Tech-MeetUp-2024-Ops-Dev-Team-7.png)
...
* `에러 대응 프로세스`를 다음과 같이 진행한다.
    ...
    * 발생 빈도가 높지 않거나 핵심 기능에 미치는 영향이 적다면 바로 대응하지 않고 모니터링을 걸어둔 채, 현재 하고 있던 작업을 빠르게 마치도록 한다.
    ...
    ![](/assets/img/server/technology/daanggn/Daanggn-Tech-MeetUp-2024-Ops-Dev-Team-8.png)
    ...
    * 그 과정에서 현재 작업을 완료하는 것이 더 중요하다면, 현재 작업을 완료하고 에러를 대응하기로 한다.
    ...
    ![](/assets/img/server/technology/daanggn/Daanggn-Tech-MeetUp-2024-Ops-Dev-Team-9.png)
