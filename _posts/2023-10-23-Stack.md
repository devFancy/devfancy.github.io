...
1) `isEmpty(), isFull()` : 각각 큐가 비었는지, 가득 찼는지를 boolean 형태로 반환한다.
2) `enqueue()` : 큐에 새로운 원소를 삽입한다. 만약 가득 차 있다면 예외를 던진다.
3) `peek()` : 맨 앞에 위치한 데이터를 읽어온다.
4) `dequeue()` : 맨 앞에 위치한 데이터를 읽어오고, 해당 데이터를 큐(queue)에서 제거한다.
