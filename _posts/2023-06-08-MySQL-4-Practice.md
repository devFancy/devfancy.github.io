...
* actor 테이블에서 first_name, last_name 컬럼 조회
    * 실행 결과 행의 수는 총 200개
...
* actor 테이블에서 first_name과 last_name을 하나로 연결(concat)하여 
    * Actor Name이라는 컬럼명으로 조회하고, 전부 대문자로 조회
    * 실행 결과 행의 수는 총 200개
...
* actor 테이블에서 actor_id, first_name, last_name을 조회하되, 
    * first_name이 Joe인 데이터만 필터링하여 조회
    * 실행 결과 행의 수는 1개
...
* actor 테이블에서 actor_id, first_name, last_name을 조회하되, 
    * last_name 에 Gen이 포함된 actor를 필터링하여 조회
...
* country 테이블에서 country_id, country 열을 조회하되, 
...
* actor 테이블에서 서로 동일한 last_name을 사용하는 배우(actor)들이 각각 몇 명인지 조회하고 싶을 때,
...
* actor 테이블에서 last_name 컬럼과 해당 last_name을 사용하는 수를 집계해주는 컬럼을 조회하되, 
...
* address 테이블의 정보(description) 조회
...
* address 테이블의 가상 상위 데이터 5개만 제한(LIMIT)하여 조회
    * 실행 결과 행의 수는 총 5개
...
* staff 테이블의 별칭을 s, address 테이블의 별칭을 a로 짓고, 
...
* staff 테이블의 별칭을 s, payment 테이블의 별칭을 p로 짓고, 
...
* film 테이블의 별칭을 f, film_actor 테이블의 별칭을 fa로 짓고, 
...
* inventory 테이블의 정보(description) 조회
...
* film 테이블에서 title, description 컬럼을 조회하되, 상위 20개만 조회
    * 실행 결과 행의 수는 총 20개
...
* ALABAMA DEVIL film이 모든 영화 대여점에 총 몇 개의 복제본(영화 필름)이 배포되어있는지 알고 싶을 때,
...
* 고객 별 총 사용 금액을 last_name을 오름차순 정렬하여 조회하고 싶을 때,
