...
또한 `Jmeter`는 단계별 스레드 할당 방식이라고 해서 스레드가 하나 생성될 때마다 리소스를 새로 할당해야 한다. 그래서 하나의 Worker에서 1000개 이상의 스레드를 할당하게 되면 조금 무리가 있을 수 있다.
즉, 한 대의 Wokrer에서 사용할 수 있는 사용자 수가 **매우 한정적**이라는 말이다.
...
나는 100, 10, 5 순으로 입력했다. -> 동시 사용자 100명이 10초동안 5번 반복하도록 실행한다.
...
> 최대 TPS = (Number of Threads X Loop Count) / Ramp-up period

위의 식에 따라 10초동안 100번의 요청을 5번 반복하도록 테스트를 구성한다면, 최대 TPS 값을 100 * 5 / 10 = 50이 된다.

그래서 초당 실행되는 스레드 수를 점차적으로 늘려가면서 테스트를 진행했을 때 어느순간 TPS 값이 증가하지 않는 결과가 나타난다.

그때의 값이 해당 요청에 대한 정확한 TPS 측정값이라고 볼 수 있다.
