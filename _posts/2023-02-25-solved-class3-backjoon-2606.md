...
* 결론적으로 **알고리즘 흐름도**는 다음과 같다.
    ...
    [9] 입력 및 초기화 -> [1] graph에 연결 정보 채우기(인접 행렬에 값 넣어주기) -> [2] dfs(재귀함수) 호출하기 -> [3] 출력하기
...
* 입력받을 변수 x,y 를 만들어주고, i=0부터 M보다 작을때까지 반복한다.
    ...
    그리고 그래프에 간선 정보를 반영하기 위해서 true를 이용한다. => `graph[x][y] = true`, `graph[y][x] = true;` (x번 컴퓨터와 y번 컴퓨터가 서로 연결되어 있기 때문에 2번 적어주는 것이다)
...
* 그리고 지금의 인덱스를 기준으로 **나와 연결되는 번호가 1번부터 N번까지 몇개 있는지 `반복문`을 통해 확인**한다.  => `for(int i = 1; i <= N; i++)`
    ...
    여기서 `if(visited[i] == false)`을 통해 이미 방문한 적이 없는지 확인하기 위해 반복문 바로 뒤에 작성한다.
