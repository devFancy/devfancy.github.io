...
### 4-2-2. 태그는 카테고리 한 곳에만 속한다 (v11에서 신설)

같은 태그가 두 카테고리에 걸리면 `/category/` 에서 같은 칩이 두 번 보인다.
태그 5종이 그런 상태였다. 태그가 주제를 더 정확히 말해주면 **글을 그 태그의 집으로 보낸다.**

| 태그 | 처리 |
|---|---|
| `#네트워크` | REST·쿠키/세션·HTTPS·NGINX 4편을 서버 → **CS**. 특정 서버 기술이 아니라 웹 공통 지식이다 |
| `#Git` | Branch Protection 글 1편을 서버 → **도구** |
| `#글또` | 글쓰기 파이프라인 1편을 기타 → **회고** |
| `#우아한테크코스` | BE 미션 회고 4편을 프로젝트 → **서버** |
| `#후기` | Coursera 수료증을 `#커리어` 로 분리. `#후기` 는 서버 전용 |
| `#사이드프로젝트` | 기술 글로 분류된 Goodfriends·Hibit **8편에서 태그를 뺀다.** 프로젝트 전용 |

규칙은 `TAG_HOME` 으로 변환 스크립트에 있다.

**"프로젝트" 는 주제가 아니라 맥락이다.** Jekyll 의 `Side_Project SpringBoot` 8편은
주제 카테고리(서버)를 쓰고 프로젝트는 떨어뜨린다. 맥락은 `#사이드프로젝트` 태그로 남는다.
이 규칙으로 **카테고리가 2개인 글이 0편**이 됐다.

**한 글이 여러 카테고리를 갖지 않는다.** 319편 전부 카테고리 하나다.

`#공모전`·`#학습`·`#커리어` 는 각각 1편뿐이라 `#커리어` 하나로 합쳤다.

태그 나열 순서는 `TAG_ORDER` 배열 하나에서 정한다. 여기 없는 태그는 뒤에 글 많은 순으로 붙는다.
...
| 인기순 / 조회수 | GA4 조회수로 빌드 시점에 산출한다 (4-4-6) |
| 댓글 | 없음 |
...
#### 4-4-5. 홈은 섹션을 쌓는다 (v11에서 신설)

319편을 한 페이지에 쏟으면 첫 화면이 목록으로 덮인다. 홈은 **최근 글 3편**만 보여주고
전체는 `/archive/` 와 `/category/` 가 맡는다. 문제풀이는 한 번에 여러 편이 올라와
최근 글을 독점하므로 홈에서 뺀다 (4-2).

대표 글·인기 글 섹션은 같은 모양으로 이 아래에 덧붙인다.

#### 4-4-6. 인기 포스트는 GA4 조회수로 만든다 (v11에서 신설)

**전제: GA4 태그가 아직 Astro 에 없다.** Jekyll 은 `_config.yml` 의
`google_analytics_id: G-7BMWW1711K` 로 gtag 를 심었는데, 이관본에는 빠져 있다.
**이대로 배포하면 통계가 끊긴다.** Phase 5 에서 먼저 태그를 이식한다.

방식은 **빌드 시점 주입**이다. 방문자 쪽에 추가 요청이 없고, 배포할 때마다 순위가 갱신된다.

| 단계 | 내용 |
|---|---|
| 1 | Google Cloud 에서 서비스 계정을 만들고 GA4 속성에 **뷰어** 권한을 준다 |
| 2 | 키(JSON)를 GitHub Actions 시크릿 `GA_SERVICE_ACCOUNT` 에 넣는다 |
| 3 | 빌드 전에 GA4 Data API 로 최근 90일 `screenPageViews` 상위 N개 경로를 받는다 |
| 4 | 경로를 글 slug 로 바꿔 `src/data/popular.json` 으로 쓴다 |
| 5 | 홈이 그 파일을 읽어 "인기 글" 섹션을 그린다 |

**API 호출이 실패해도 빌드는 성공해야 한다.** 키가 없거나 할당량이 넘치면
직전 `popular.json` 을 그대로 쓰고 경고만 남긴다. 통계 때문에 배포가 막히면 안 된다.

제외 대상: 문제풀이(4-2), `/archive/` `/category/` 같은 목록 페이지, 404.

대안으로 검토했다가 접은 것:

| 안 | 접은 이유 |
|---|---|
| GA4 대시보드를 보고 손으로 목록 고정 | 자동 갱신이 안 된다. 분기마다 사람이 해야 한다 |
| 자체 조회수 카운터 (Cloudflare Workers 등) | 실시간이지만 인프라가 하나 늘고 봇 필터링을 직접 해야 한다 |
...
|---|---|
| 대상 | **1238개 파일** / 586MB |
| 도구 | **`sharp`** (v10에서 변경). Astro 의존성으로 이미 있어 추가 설치가 없다 |
| 손실 여부 | 항목별로 다르다. 8-3-3 참조 |
| 기대 절감 | 8-3-3 실측 참조 |
...
#### 8-3-2-1. 이미지 이관 결과 (v11에서 신설)

`assets/img` 를 `public/assets/img` 로 **`git mv` 로 옮겼다.** 복사하면 586MB 가 저장소에
추가된다. 1,238 개 전부 이미 git 이 추적하던 파일이라 이동은 크기를 늘리지 않는다.

약어 폴더 5개만 이름을 편다 (B안). 나머지 34개는 그대로 둔다.

| 이전 | 이후 |
|---|---|
| `bs/` | `business-statistics/` |
| `ef/` | `electronic-finance/` |
| `ps/` | `probability-statistics/` |
| `goodcode/` | `clean-code/` |
| `record/` | `dev-history/` |

본문 경로 치환은 `IMAGE_DIR_RENAME` 으로 변환 스크립트에 넣었다. 수작업이 아니다.

`astro.config.mjs` 의 `legacyJekyllImages` dev 전용 플러그인은 제거했다.

**이관 중 찾은 결함 3건**

| 대상 | 문제 | 처리 |
|---|---|---|
| `Technology-Woowa-Tech-Talk-...` | `/assets/img/tech_insight/` 는 존재한 적 없는 폴더. 같은 글의 다른 23장은 `technology/woowabros/` 에 있다. **원본 Jekyll 에서도 깨져 있었다** | `IMAGE_PATH_FIX` |
| `2023-Retrospective` | `http://127.0.0.1:4000/` Jekyll 개발 서버 주소 2건이 링크로 배포돼 있었다 | `LINK_FIX`. `#Side_Project` 는 `#사이드프로젝트` 로 |
| Dev History 4편 | `<script>` Gist 임베드가 본문에 프레임으로 그려진다 | `EMBED_REMOVE` (정규식이라 연도가 늘어도 적용) |

**고아 파일 88개**는 지우지 않았다. 참조가 없을 뿐 지우는 것은 별도 판단이다.

#### 8-3-3. 압축 실측과 미결 (v10에서 신설)

**단일 방식으로는 답이 없다.** 내용에 따라 정반대 결과가 나온다.

| 대상 | 원본 | PNG 팔레트 | JPEG 82 | 무손실 재압축 |
|---|---|---|---|---|
| 도표·스크린샷 (표본 11) | 1.6MB | **0.55MB** | 0.70MB | 1.5MB |
| 사진 1장 (2980×3973) | 12.7MB | 6.5MB | **1.5MB** | 13.9MB |

- 도표·스크린샷은 팔레트가 JPEG보다 **1.5~2.6배 작다.** 글자 가장자리도 JPEG가 더 뭉갠다
- 사진은 반대로 JPEG가 4.5배 작다
- **무손실 재압축은 사진에서 파일을 키운다** (10MB 초과 13개 합계 155MB → 190MB, 123%).
  아이폰 사진을 PNG 로 담아둔 것이라 PNG 재인코딩이 원본 인코더보다 불리하다

**진짜 지렛대는 해상도다.** 10MB 초과 13개는 3024×4032 급인데 본문 폭은 768px 이다.
1600px 로 줄이면 **155MB → 19MB (12%)** 이고 화면상 차이가 없다.

| 상한 | 결과 |
|---|---|
| 1600px | 155MB → 19MB (12%) |
| 2000px | 155MB → 30MB (19%) |

**미결.** 확장자 유지(11절)와 사진 화질 유지를 둘 다 지키려면 해상도 축소가 유일한 수단인데,
원본을 클릭해 크게 보는 경우에는 차이가 생긴다. 적용 시점과 상한값은 사람이 정한다.

`oxipng` 같은 무손실 도구는 스크린샷에서 10~30%밖에 줄지 않아 목적에 미달한다.

#### 8-3-4. 확장자와 실제 형식 불일치 35건 (v10에서 신설)

`.png` 인데 내용은 다른 형식인 파일이 35개 있다.

| 실제 형식 | 개수 | 영향 |
|---|---|---|
| JPEG | 28 | 브라우저가 내용으로 판단해 정상 표시된다. 그대로 둔다 |
| WebP | 4 | 정상 표시된다 |
| GIF | 1 | 정상 표시된다 |
| **HEIC** | **2** | **크롬·파이어폭스가 렌더하지 못한다** |

HEIC 2개는 `assets/img/goodcode/GoodCode-BadCode-2_1.png`, `..._2.png` 다.
**319편 어디에서도 참조하지 않는 고아 파일**이라 실제 피해는 없다. 이관 대상에서 뺀다.

압축 스크립트는 확장자가 아니라 **실제 형식으로 분기해야 한다.** `.png` 라고 믿고
PNG 인코더에 넣으면 이 파일들에서 깨진다.
...
| 구분 | 건수 | 판정 |
|---|---|---|
| 페이지네이션 `/page2/`~`/page64/` | 63 | 의도된 제거 (4-1) |
| 이미지 | 102 | 98개는 폴더 개명(8-3-2-1), 3개는 스냅샷이 NFD 로 기록된 오탐 |
| 정적 파일 (`main.css`, `search.json`, `LICENSE` 등) | 18 | 의도된 제거 |
| `/search/` | 1 | `/category/` 로 통합 (4-4-2) |
| `LICENSE` · `README.md` · `main.css.map` | 3 | Jekyll 이 복사하던 저장소 파일 |

**여기서 찾은 결함 1건**

`DB-Normalization.md` 가 `DB-Normalization-4.jpg` 를 참조하는데 실제 파일은 `.JPG` 다.
**macOS 는 대소문자를 구분하지 않아 로컬에서는 멀쩡하지만 GitHub Pages(리눅스)는 404 를 낸다.**
원본 Jekyll 에도 있던 버그다. 참조 1,170건을 전수 대조해 이 1건만 나왔고 `IMAGE_PATH_FIX` 로 고쳤다.

> 대소문자와 유니코드 정규화는 로컬 빌드로는 잡히지 않는다. **git 인덱스에 저장된 바이트**와
> 본문 참조를 대조해야 한다.
...
| v10 | 2026-09-20 | Phase 3 마크다운 변환 완료(319편, URL 누락 0). 압축 도구를 pngquant → sharp 로 변경하고 실측을 8-3-3 에 신설(무손실이 사진을 키운다, 해상도가 진짜 지렛대), 확장자/실제 형식 불일치 35건과 HEIC 2건을 8-3-4 에 신설, 저장소를 iCloud 동기화 폴더 밖으로 이동 |
