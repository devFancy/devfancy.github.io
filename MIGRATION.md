...
| 버전 | 날짜 | 내용 |
|---|---|---|
| v11 | 2026-09-20 | Phase 3 완료. 이미지 1,238개를 `git mv` 로 `public/assets/img` 이관(8-3-2-1), 약어 폴더 5개 개명, dev 전용 이미지 플러그인 제거. 태그를 카테고리 한 곳에만 두는 규칙 신설(4-2-2)로 카테고리 2개인 글 0편·태그 중복 0종 달성. `/category/`·`/archive/`·`/solutions/` 구현과 홈을 최근 3편으로 축소(4-4-5). 원본에서 깨져 있던 이미지 경로·개발 서버 링크·Gist 임베드 3종 교정 |
| v1 | 2026-09-20 | 최초 작성 |
